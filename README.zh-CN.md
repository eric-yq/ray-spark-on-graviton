...
一套可复现的大数据 benchmark,用于对比 AWS **Intel(m7i、m8i)** 与 **Graviton(m8g、m9g)**
实例在两种引擎上的性能:
...
每种架构跑完全对称的集群:**1 个调度/head + 3 个任务/worker 节点**,用 Ray 原生 cluster
launcher(`ray up`)拉起。各套集群**只有实例族不同** —— 软件版本、实例规格、EBS 配置、
...
\* 1 head + 3 worker,us-east-1 Linux 按需(用户 2026-07 提供的数值;可用
`BENCH_PRICE_<INSTANCE>` 按实例覆盖)。报告默认以 m7i 为基线。
...
`launch.sh` 接受 **`m7i` | `m8i` | `m8g` | `m9g`**(自动解析对应的 x86_64/arm64 AMI)。
想对比哪些族,就各跑一遍完整流程。
...
在控制机上执行(从 S3 拉取所有集群的结果):

```bash
python scripts/report.py --from-s3 s3://your-bucket/results   # 基线默认 m7i
python scripts/report.py --from-s3 s3://your-bucket/results --baseline m8g
```
产出 `results/comparison.csv` 和 `.md`,每 (负载, 架构) 一行;每个非基线架构都与基线(默认 m7i)对比:

| 列 | 含义 |
|------|------|
| `best_s` / `median_s`  | `--repeat` 多次里的最优 / 中位墙钟 |
| `cost_usd`             | 该最优 run 的按需成本(整集群 $/hr × best_s) |
| `speedup_vs_m7i`       | `m7i_best / arch_best` —— **> 1 表示比 m7i 快** |
| `priceperf_vs_m7i`     | `m7i_cost / arch_cost` —— **> 1 表示每美元干的活更多** |

(基线行按定义都是 1.0。用 `--baseline` 换成别的族作基线。)
...
| `BENCH_ARCH`             | 自动(EC2 元数据) | 强制架构标签:`m7i`/`m8i`/`m8g`/`m9g` |
