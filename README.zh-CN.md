...
| 组件      | 版本    | 说明 |
|-----------|---------|------|
| Python    | 3.10    | Ubuntu 22.04 系统 python;`python3` 即 3.10 |
...
| PyArrow   | 17.0.0  | |
| JDK       | Amazon Corretto 17 | apt 源,两架构通用 |
...
为每个架构解析 Ubuntu 22.04 AMI,填入 `ImageId` 字段:
...
- **Spark 的 python worker 报 `ModuleNotFoundError: No module named 'pyspark'/'ray'`**:
  executor 的 `python3` 必须能 import ray + pyspark + pyarrow + raydp。集群上 `python3` 即
  3.10(node_setup 把所有东西装在那里);集群 YAML 也在 `ray start` 上固定了
